(function () {
  ...
  function mapCourse(row) {
    ...
    return {
      id: String(row.id || ''),
      title: String(d.courseTitle || d.title || row.name || 'Course'),
      status: String(d.status || row.status || 'active'),
      category: String(d.category || 'General'),
      level: String(d.level || 'Intermediate'),
      price: num(d.price || d.coursePrice, 49),
      instructor: String(d.instructor || d.instructorEmail || row.userId || 'Instructor'),
      raw: row,
    };
  }
  ...
  function renderDashboard() {
    var uniqueLearners = new Set(state.enrollments.map(function (e) { return e.learnerKey || e.id; })).size;
    var uniqueInstructors = new Set(state.courses.map(function (c) { return c.instructor; })).size;
    var totalUsers = uniqueLearners + uniqueInstructors + 1;
    var fallbackRevenue = state.enrollments.reduce(function (sum, e) {
      var course = state.courses.find(function (c) { return c.title === e.title; });
      return sum + (course ? course.price : 39);
    }, 0);
    var paymentRevenue = state.payments.reduce(function (sum, p) { return sum + p.amount; }, 0);
    ...
    setText('metricEnrollments', state.enrollments.length);
    setText('metricRating', (state.courses.length ? 4.5 : 0).toFixed(1));
  }
  ...
  function renderUsers() {
    ...
    state.courses.forEach(function (c) {
      ...
      if (map[key]) return;
      map[key] = { key: key, email: c.instructor, role: 'instructor' };
    });
    state.enrollments.forEach(function (e) {
      ...
      if (map[key]) return;
      map[key] = { key: key, email: email, role: 'student' };
    });
  ...
  function renderCourses() {
    ...
    table.innerHTML = state.courses.length
      ...
      : '<tr><td colspan="3">No course records found.</td></tr>';
  }

  function renderCategories() {
    var list = document.getElementById('categoriesList');
    if (!list) return;
    var categories = Array.from(new Set(state.courses.map(function (c) { return c.category; }).filter(Boolean)));
  ...
  function renderRevenue() {
    ...
    if (state.payments.length) {
      ...
      state.enrollments.forEach(function (e, idx) {
        var c = state.courses.find(function (x) { return x.title === e.title; });
  ...
  function renderReports() {
    ...
    list.innerHTML = [
      ...
      'Top course by enrollment: ' + (topCourse ? topCourse.title : 'N/A'),
  ...
  function renderSettings() {
    ...
    if (!list) return;
    list.innerHTML = '<li>Platform mode: Production</li><li>Certificate template: LMS v1</li><li>Payment gateway: Stripe (sandbox)</li>';
  }
  ...
  function bindUsersActions() {
    ...
    table.addEventListener('click', async function (event) {
      ...
      var role = String(btn.getAttribute('data-user-role') || 'student');
  ...
  function bindCourseActions() {
    ...
    table.addEventListener('click', async function (event) {
      ...
      if (!out.res || !out.res.ok) {
        window.LMS_AUTH.showToast((out.data && out.data.message) || 'Unable to update course');
        return;
      }
      window.LMS_AUTH.showToast('Course status updated');
      refreshData();
    });
  }
