(function () {
  ...
  function normalizeRole(role) {
    ...
    if (!raw || raw === 'user' || raw === 'customer' || raw === 'patient') return 'student';
    if (raw === 'manager' || raw === 'teacher') return 'instructor';
    return raw;
  }
  ...
  function currentUser() {
    ...
    var role = normalizeRole(stored.role || decodeRoleFromToken(token) || 'student');
    return {
      token: token,
      role: role,
  ...
  function pathForRole(role) {
    ...
    if (r === 'instructor') return 'instructor/dashboard.html';
